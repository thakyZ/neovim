...
--- Special value representing NIL in RPC and v:null in Vimscript conversion, and similar cases. Lua `nil` cannot be used as
--- part of a Lua table representing a Dictionary or Array, because it is treated as missing: `{"foo", nil}` is the same as
--- `{"foo"}`.
---@type userdata
vim.NIL = nil

--- Type index for use in lua-special-tbl. Specifying one of the values from {@see vim.types} allows typing the empty table (it
--- is unclear whether empty Lua table represents empty list or empty array) and forcing integral numbers to be Float. See
--- lua-special-tbl for more details.
---@type boolean
vim.type_idx = true
...
--- Table with possible values for {@see vim.type_idx}. Contains two sets of key-value pairs: first maps possible values for
--- {@see vim.type_idx} to human-readable strings, second maps human-readable type names to values for {@see vim.type_idx}.
--- Currently contains pairs for `float`, `array` and `dictionary` types.
---
--- Note: One must expect that values corresponding to `vim.types.float`, `vim.types.array` and `vim.types.dictionary` fall
--- under only two following assumptions: 1. Value may serve both as a key and as a value in a table. Given the properties of
--- Lua tables this basically means “value is not `nil`”. 2. For each value in `vim.types` table `vim.types[vim.types[value]]`
--- is the same as `value`. No other restrictions are put on types, and it is not guaranteed that values corresponding to
--- `vim.types.float`, `vim.types.array` and `vim.types.dictionary` will not change or that `vim.types` table will only contain
--- values for these three types.
---@class table
vim.types = {
  [6] = "dictionary",
  dictionary = 6,
...
--- Creates a special empty table (marked with a metatable), which Nvim converts to an empty dictionary when translating Lua
--- values to Vimscript or API types. Nvim by default converts an empty table `{}` without this metatable to an list/array.
---
--- Note: If numeric keys are present in the table, Nvim ignores the metatable marker and converts the dict to a list/array
--- anyway.
---@return table
function vim.empty_dict() end


--- The result is a String, which is the text {@paramref str} converted from encoding {@paramref from} to encoding
--- {@paramref to}. When the conversion fails `nil` is returned. When some characters could not be converted they are replaced
--- @with "?". The encoding names are whatever the iconv() library function can accept, see ":Man 3 iconv".
---@param str string Text to convert
---@param from string Encoding of {@paramref str}
---@param to string Target encoding
---@return string? #Converted string if conversion succeeds, `nil` otherwise.
function vim.iconv(str, from, to) end
...
---Sends a request to {@paramref channel} to invoke {@paramref method} via RPC and blocks until a response is received.
---
---Note: NIL values as part of the return value is represented as {@see vim.NIL} special value
---@param channel integer
---@param method string
---@param ... any?
function vim.rpcrequest(channel, method, ...) end

---Sends `{event}` to {@paramref channel} via RPC and returns immediately. If {@paramref channel} is 0, the event is broadcast
---to all channels.
---
