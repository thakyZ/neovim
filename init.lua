...
--[[ Disabled, originated from the original `init.lua` script
...
local plugins = {
  ...
  },
  {
    ...
    opts = { -- Configure core features of AstroNvim
      features = {
        ...
        autopairs        = true, -- enable nvim-various-textobjs-pairs at start
        cmp              = true, -- enable completion at start
        large_buf        = {size= 1024 * 500, lines = 10000 }, -- set global limits for large files for disabling features like Tree-sitter
      },
  ...
  { -- Optional cmp completion source for require statements and module annotations
    "hrsh7th/nvim-cmp",
    ---@module 'nvim-cmp'
    ---@param _    unknown    --- An unknown parameter
    ---@param opts cmp.Config --- The opts for this package
    ---@return unknown|nil
    opts = function(_, opts)
  ...
  },
  {
    ...
    lazy = false,
  }, -- Permanent solution until fix https://discord.com/channels/939594913560031363/1088835559012716584
  {
    "Shatur/neovim-ayu",
    ---@module 'ayu'
    config = function()
      -- `local utils = require "default_theme.utils"`
      require("ayu").setup({                             -- don't forgor `:PackerCompile` if it doesn't work
    ...
    config = function () -- #2
      require("nvim-treesitter.configs").setup = {
        yati = {
          ...
          default_lazy     = true, -- Whether to enable lazy modee (recommend to enable this if bad indent happens frequently)
  ...
  ]]--
  {
    ...
    -- Customize how the language servers are attached
    handlers = {
      ---@param _    unknown
      ---@param opts table
      ---@return unknown|nil
    ...
    opts = {
      features = {
        ...
        codelens        = true , -- Enable or disable code-lens refresh on start
...
local polish = function()
  ...
  map.set('n', '<leader>gd', ':Gdiffsplit<CR>'   )
  map.set('n', '<leader>gc', ':Git commit<CR>'   )
  map.set('n', '<leader>gb', ':Git blame<CR>'    )
  map.set('n', '<leader>gm', ':Git mergetool<CR>')
  ...
  elseif vim.g.colors_name == "sunbather" or vim.g.colors_name == "onedark" then
    if _G.IS_WINDOWS then
